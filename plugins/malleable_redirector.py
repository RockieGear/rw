...
''')
    ...
    def strip_headers(self, req, malleable_meta):
        ...
        if 'http-config' in self.malleable.config.keys() and 'trust_x_forwarded_for' in self.malleable.config['http-config'].keys():
            if self.malleable.config['http-config']['trust_x_forwarded_for'] == True or self.malleable.config['http-config']['trust_x_forwarded_for'] == "True":
                self.logger.info('X-FORWARDED-FOR PRESENT!!!!', color='green')
    ...
    def _request_handler(self, req, req_body):
        ...
        self.logger.info(f'Peer IP: {peerIP}')
        req.headers['x-forwarded-for'] = f'{peerIP}'
